...
### 贝叶斯意图推断模块优化 & 泛化基线实验启动

---

#### 22. 泛化基线实验脚本 & 启动

**目的：** 量化固定 BC 先验的 DEIA 在非 BC 队友（SP/FCP/MEP）上的得分下降幅度，为第二个点（在线适应）提供实验动机

**创建的文件：**
- `src/run_generalization_baseline.sh` — 5 layout × 3 opponents × 2 position × 5 ep，共 30 组

**修复：** 脚本中 `python3` 改为 `/home/aj/miniconda3/envs/proagent/bin/python3`（conda run 在 WSL2 下无法正确激活环境）

**实验状态：** 后台运行中，已完成 22/30 组

**初步结论：**
- MEP 最兼容（跨场景均值 ~206），DEIA 与 MEP 配合甚至超过 BC 水平
- FCP 中等（均值 ~170），大多数场景接近 BC 水平
- SP 最差（均值 ~141），forced_coordination SP_DEIA_p1 仅 16 分，直接验证固定先验的局限
...
**问题：**
1. 任务切换检测过于敏感：捡起/放下物品都触发软重置，导致信念无法积累
2. 衰减（0.95）+ 最大概率上限（0.90）双重机制互相削弱，信念难以收敛

**修改的文件：**
- `src/proagent/itdp_module.py`

**具体改动：**
1. `_detect_task_switch()`：删除"手持从有到无"和"从无到有"两个触发条件，只保留物品类型切换（ingredient↔plating）和拿到 soup 的触发
2. `update()`：衰减系数 0.95 → 0.98，减少每步向均匀分布靠拢的幅度
3. `update()`：删除最大概率上限（max_prob=0.90）逻辑，让信念在队友行为一致时能自然收敛到更高置信度
