...
# 支持多种API key文件名
SILICONFLOW_KEY_FILE = os.path.join(cwd, "siliconflow_key.txt")
...
class ProAgent(object):
    ...
    def load_openai_keys(self):
        ...
        if os.path.exists(key_file):
            ...
            print(f"[INFO] Loaded {len(self.openai_api_keys)} {key_type} API key(s) from {key_file}")
        else:
            # 尝试从环境变量获取
            env_key = os.environ.get("SILICONFLOW_API_KEY") if self._is_siliconflow_model() else os.environ.get("OPENAI_API_KEY")
            if env_key:
                self.openai_api_keys = [env_key]
                print(f"[INFO] Loaded {key_type} API key from environment variable")
            else:
                raise FileNotFoundError(
                    f"API key file not found: {key_file}\n"
                    f"Please create the file or set the environment variable:\n"
                    f"  - For SiliconFlow: SILICONFLOW_API_KEY\n"
...
class ProMediumLevelAgent(ProAgent):
    """
    This agent uses LLM to generate medium level actions.
    默认使用硅基流动的 Qwen/Qwen2-7B-Instruct 模型
    """
    def __init__(
            self,
            mlam,
    ...
    def create_gptmodule(self, module_name, file_type='txt', retrival_method='recent_k', K=10):
        print(f"\n--->Initializing LLM {module_name} with model: {self.model}<---\n")    
    ...
    def parse_ml_action(self, response, agent_index): 
        ...
        # avoid to generate two skill, eg, Plan for Player 0: "deliver_soup(), pickup(onion)".
        if "," in ml_action:
    ...
    def get_lowest_cost_action_and_goal_new(self, start_pos_and_or, motion_goals, state): 
        """
        Chooses motion goal that has the lowest cost action plan with collision avoidance.
        """   
        min_cost = np.Inf
        best_action, best_goal = None, None
        for goal in motion_goals:   
