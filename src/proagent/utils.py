...
def retry_with_exponential_backoff(
    ...
    支持 OpenAI 和 硅基流动 API 的错误处理
    """
    # 动态设置错误类型，兼容新旧版本openai库
    if errors is None:
        try:
            # 旧版openai库 (< 1.0)
            errors = (openai.error.RateLimitError, openai.error.APIError, openai.error.Timeout)
        except AttributeError:
            # 新版openai库 (>= 1.0)
            try:
                errors = (openai.RateLimitError, openai.APIError, openai.Timeout)
            except AttributeError:
                # 如果都不存在，使用通用Exception
                errors = (Exception,)
    ...
    def wrapper(*args, **kwargs):
        ...
        while True:
            try:
                ...
                print(f"[Retry {num_retries + 1}/{max_retries}] Error: {e}")
                num_retries += 1
