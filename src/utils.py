...
def make_agent(alg:str, mdp, layout, **gptargs):
    ...
    if alg == "Stay":
        ...
        if layout in OLD_LAYOUTS:
            ...
            print(f"[WARNING] BC agent not available for custom layout '{layout}', using Greedy instead")
            MLAM_PARAMS = {
        ...
        if layout in OLD_LAYOUTS:
            ...
            print(f"[WARNING] {alg} agent not available for custom layout '{layout}', using Greedy instead")
            MLAM_PARAMS = {
                "start_orientations": False,
                "wait_allowed": True,
                "counter_goals": [],
